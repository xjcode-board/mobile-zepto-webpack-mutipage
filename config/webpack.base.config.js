...
const baseConfig = {
  entry: glob.sync(resolve(__dirname, '../src/pages/**/*.js')).filter(v => !v.includes('common')).reduce((pre, filepath) => {
    const tempList = filepath.split('pages/')[1].split(/\//)
    const filename = `${tempList[1].replace(/\.js/g, '')}`
  ...
  plugins: [
    ...
    ...glob.sync(resolve(__dirname, '../src/pages/**/*.ejs')).filter(v => !v.includes('common')).map((filepath, i) => {
    ...
    new CopyWebpackPlugin([{
      ...
      to: resolve(__dirname, `../${config.outputPath}/lib`)
    }]),
